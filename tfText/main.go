...
func printTF(dataMap map[string]int) {
	fmt.Println("======================")
...
func tf(text string) map[string]int {
	...
	mapWordCoutnInText := make(map[string]int)

	for _, word := range dataWordText {
		mapWordCoutnInText[word]++
	}
	return mapWordCoutnInText
}
